...
class TerminatorThemes(plugin.Plugin):
    ...
    def configure(self, widget, data=None):
        ...
        main_container.pack_start(
            self._create_settings_grid(ui), True, True, 0)
    ...
    def _create_themes_grid(self, ui):
        ...
        grid.attach_next_to(buttons[0], scroll_window,
                            Gtk.PositionType.BOTTOM, 1, 1)

        for i, button in enumerate(buttons[1:]):
            grid.attach_next_to(
                button, buttons[i], Gtk.PositionType.RIGHT, 1, 1)
    ...
    def _create_settings_grid(self, ui):
        ...
        grid.attach(self._create_main_action_button(
            ui, "install", self.on_install), 0, 4, 1, 1)
        grid.attach(self._create_main_action_button(
            ui, "remove", self.on_uninstall), 1, 4, 1, 1)
    ...
    def _create_inherits_from_combo(self, ui):
        ...
        combo.set_active(self.profiles.index(
            self.terminal.config.get_profile()))
